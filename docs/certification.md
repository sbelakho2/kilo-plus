...
| VSIX packaging (P0) | pinned vendored closure + companion overlay staged under `media/kilo-v756-webview`; extensionUri-only resolution; package → unzip → hash verify → IDE-load record | `apps/vscode/scripts/prepare-vendored-webview.mjs`, `apps/vscode/scripts/verify-vsix.mjs`, CI `vscode-build` | CLOSED (self-contained VSIX; the IDE-load step records its exact skip when no `code` CLI exists) |
