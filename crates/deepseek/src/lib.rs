...
mod tests {
    ...
    /// Shared canonical-usage conformance for the DeepSeek wire (audit
    /// Phase-1 item C). DeepSeek's native API is the OpenAI Chat
    /// Completions shape: `usage.prompt_tokens` is the TOTAL input
    /// INCLUDING the cached portion (DeepSeek additionally reports
    /// `prompt_cache_hit_tokens` / `prompt_cache_miss_tokens`, which
    /// mirror `prompt_tokens_details.cached_tokens`), completion tokens
    /// include reasoning with `completion_tokens_details.reasoning_tokens`
    /// as the informational subset, and the request id rides the frame's
    /// top level. The adapter maps its wire usage through the OpenAI chat
    /// family parser (`faktor_openai`); the frames below carry DeepSeek's
    /// extra cache fields next to the standard detail so the mapping is
    ...
    mod canonical_usage_conformance {
        ...
        /// A real-wire deepseek chat usage frame: the OpenAI-standard
        /// detail plus DeepSeek's own cache counters (hit+miss == total).
        /// `junk` adds unknown fields everywhere (never panic).
        fn usage_frame(
            prompt: u64,
            completion: u64,
            cached: Option<u64>,
            ...
            if junk {
                usage["unknown_usage_field"] = serde_json::json!({"deep": [1, 2]});
            }
        ...
        canonical_usage_conformance! {
            driver: deepseek_wire_canonical_usage_conformance,
            family: faktor_provider::usage_conformance::WireFamily::InclusiveTotal,
            label: "deepseek chat wire",
            request: || req("deepseek-v4-flash"),
            provider: |base: String| {
                let cfg = DeepSeekConfig {
                    profile: DeepSeekProfile::Compatible { base_url: base },
                    api_key: None,
                    model_overrides: std::collections::HashMap::new(),
                };
                build(cfg)
            ...
            cases: vec![
                ...
                faktor_provider::usage_conformance::WireUsageCase::frame(
                    ...
                    sse(usage_frame(1000, 50, None, None, None, true)),
                    exp(1000, 0, 50, 0, None),
                ),
                faktor_provider::usage_conformance::WireUsageCase::frame(
                    "request_id_preserved",
                    sse(usage_frame(1000, 50, None, None, Some("deepseek-call-1"), false)),
                    exp(1000, 0, 50, 0, Some("deepseek-call-1")),
                ),
            ]
        }
    }
