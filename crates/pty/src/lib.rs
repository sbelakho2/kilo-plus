//! Unix interactive-terminal support (audit round 11: PTY was the largest
//! non-blocked compatibility gap — run_command is not a terminal).
//!
...
//!
//! Windows ConPTY / Job Objects remain a declared platform blocker:
//! every operation returns `Unsupported` there.
...
#[cfg(not(unix))]
impl Pty {
    pub fn spawn(_cfg: &PtyConfig) -> Result<Self, Error> {
        Err(Error::new(
            ErrorKind::Unsupported,
            "PTYs are unsupported on this platform (ConPTY/Job Objects are a declared platform blocker)",
        ))
    }
    pub fn pid(&self) -> u32 {
        0
    }
    pub fn write_all(&self, _b: &[u8]) -> Result<(), Error> {
        Err(Error::new(ErrorKind::Unsupported, "PTYs unsupported"))
    }
    pub fn write_line(&self, _l: &str) -> Result<(), Error> {
        Err(Error::new(ErrorKind::Unsupported, "PTYs unsupported"))
    }
    pub fn resize(&self, _r: u16, _c: u16) -> Result<(), Error> {
        Err(Error::new(ErrorKind::Unsupported, "PTYs unsupported"))
    }
    pub fn size(&self) -> (u16, u16) {
        (0, 0)
    ...
    pub fn wait_for_contains(&self, _n: &str, _t: std::time::Duration) -> bool {
        false
    }
    pub fn is_alive(&self) -> bool {
        false
