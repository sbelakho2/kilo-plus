...
mod tests {
    ...
    fn permission(id: i64) -> PermissionRequest {
        PermissionRequest {
            id,
            op_id: OpId::new(1),
            capability: Capability::ExecuteShell { command: "ls".into() },
    ...
    async fn resolver_wakes_waiter() {
        ...
        let handle = tokio::spawn(async move {
            r2.request(SessionId::new(1), &permission(7)).await.unwrap()
        });
    ...
    async fn double_resolve_loses_second() {
        ...
        assert!(!r.resolve(1, PermissionDecision::Deny), "first decision wins");
    }
