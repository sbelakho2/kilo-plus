...
The frozen v7.5.6 wire contract. Golden tests in `faktor-protocol` lock
...
- `startup_line.json` — the frozen stdout line `faktor server listening on http://127.0.0.1:<port>` (replaced the legacy `handshake.json` JSON-handshake payload)
...
- `password_auth.json` — `FAKTOR_SERVER_PASSWORD` header semantics (Bearer + x-kilo-server-password; 401 shape; /global/health is public)
